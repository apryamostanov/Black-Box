...
class T_logger implements I_logger {
    ...
    private I_method_invocation PC_DEFAULT_METHOD_INVOCATION = T_const.GC_NULL_OBJ_REF as I_method_invocation
    private String p_mode = T_const.GC_EMPTY_STRING
    ...
    I_trace spawn_trace(I_trace i_trace_new, I_trace i_trace_config) {
        ...
        if (i_trace_config != T_const.GC_NULL_OBJ_REF) {
        ...
        if (p_mode == T_s.c().GC_LOGGER_MODE_PRODUCTION) {
            ...
            l_trace.set_ref(i_trace_new.get_ref())
            if (i_trace_new.get_val() != T_const.GC_EMPTY_STRING) {
                l_trace.set_val(i_trace_new.get_val())
            } else {
    ...
    void put_to_context(Object i_object, String i_name) {
        I_trace l_trace = object2trace(i_object, T_const.GC_TRACE_SOURCE_CONTEXT)
    ...
    static Boolean method_arguments_exist(Object[] i_traces) {
        return (i_traces != T_const.GC_SKIPPED_ARGS && i_traces != T_const.GC_NULL_OBJ_REF && i_traces.size() > T_const.GC_EMPTY_SIZE)
    }
    ...
    @Override
    void log_enter(String i_class_name, String i_method_name, I_trace... i_traces = T_const.GC_SKIPPED_ARGS) {
        ...
        ArrayList<I_trace> l_method_arguments = objects2traces_array(i_traces, T_const.GC_TRACE_SOURCE_RUNTIME)
    ...
    @Override
    void log_exit(String i_class_name, String i_method_name, I_trace... i_traces = T_const.GC_SKIPPED_ARGS as I_trace[]) {
        I_event l_event = create_event("exit", i_class_name, i_method_name)
        if (method_arguments_exist(i_traces)) {
            l_event.add_traces_runtime(objects2traces_array(i_traces, T_const.GC_TRACE_SOURCE_RUNTIME))
        }
        log_generic(l_event)
        pop()
    }
    ...
    @Override
    void log_exception(String i_class_name, String i_method_name, Throwable i_throwable, I_trace... i_traces = T_const.GC_SKIPPED_ARGS) {
    ...
    @Override
    void log_error(String i_class_name, String i_method_name, Throwable i_throwable, I_trace... i_traces = T_const.GC_SKIPPED_ARGS) {
        I_event l_event = create_event("error", i_class_name, i_method_name)
        l_event.set_throwable(i_throwable)
        l_event.add_traces_runtime(objects2traces_array(i_traces, T_const.GC_TRACE_SOURCE_RUNTIME))
        log_generic(l_event)
    ...
    @Override
    void log_debug(T_static_string i_static_string_message, Object... i_traces = T_const.GC_SKIPPED_ARGS as Object[]) {
    ...
    @Override
    void log_info(T_static_string i_static_string_info, Object... i_traces = T_const.GC_SKIPPED_ARGS as Object[]) {
    ...
    @Override
    void log_warning(T_static_string i_static_string_warning, Object... i_traces = T_const.GC_SKIPPED_ARGS as Object[]) {
