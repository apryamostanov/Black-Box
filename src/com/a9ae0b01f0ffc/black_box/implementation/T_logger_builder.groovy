...
class T_logger_builder extends T_logging_base_6_util {
    ...
    T_logger create_logger(GPathResult i_log_conf) {
        p_conf = i_log_conf
    ...
    T_logger create_logger(String i_conf_file_name) {
        return create_logger((GPathResult) new XmlSlurper().parse(i_conf_file_name))
    }

    static T_destination init_destination(GPathResult T_destination_xml) {
