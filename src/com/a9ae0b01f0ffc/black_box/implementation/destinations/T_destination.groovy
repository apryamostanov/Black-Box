...
abstract class T_destination extends T_inherited_configurations implements I_destination {
    ...
    I_event_formatter p_formatter = T_const.GC_NULL_OBJ_REF as I_event_formatter
    String p_purpose = T_const.GC_EMPTY_STRING
    HashMap<String, I_event> p_configuration_events_by_name = new HashMap<String, I_event>()
    String p_location = T_const.GC_EMPTY_STRING
    String p_buffer = T_const.GC_EMPTY_STRING
    Integer p_buffer_size
    String p_spool_event = T_const.GC_EMPTY_STRING
    ...
    static I_trace init_predefined_trace(String i_predefined_trace_name) {
        ...
        l_trace.set_source(T_const.GC_TRACE_SOURCE_PREDEFINED)
    ...
    static I_trace find_and_build_trace_inclusive(I_event i_event_runtime, I_trace i_trace_config) {
        ...
        if (l_trace_config_source == T_const.GC_EMPTY_STRING || l_trace_config_source == T_const.GC_TRACE_SOURCE_ALL) {
        ...
        if (l_trace_config_source == T_const.GC_EMPTY_STRING || l_trace_config_source == T_const.GC_TRACE_SOURCE_ALL || l_trace_config_source == T_const.GC_TRACE_SOURCE_RUNTIME) {
        ...
        if (l_trace_config_source == T_const.GC_EMPTY_STRING || l_trace_config_source == T_const.GC_TRACE_SOURCE_ALL || l_trace_config_source == T_const.GC_TRACE_SOURCE_CONTEXT) {
        ...
        if (l_trace_config_source == T_const.GC_EMPTY_STRING || l_trace_config_source == T_const.GC_TRACE_SOURCE_ALL || l_trace_config_source == T_const.GC_TRACE_SOURCE_EXCEPTION_TRACES) {
            if (i_event_runtime.get_throwable() != T_const.GC_NULL_OBJ_REF) {
                if (i_event_runtime.get_throwable() instanceof E_application_exception) {
                    for (I_trace l_trace_from_exception : T_s.l().objects2traces(((E_application_exception) i_event_runtime.get_throwable()).get_traces(), T_const.GC_TRACE_SOURCE_EXCEPTION_TRACES)) {
    ...
    static I_trace build_predefined_trace(I_trace i_predefined_trace, I_event i_event_runtime, I_trace i_trace_config) {
        ...
        l_result_trace.set_val(T_const.GC_EMPTY_STRING)
        l_result_trace.set_source(T_const.GC_TRACE_SOURCE_PREDEFINED)
        l_result_trace.set_name(i_predefined_trace.get_name())
        l_result_trace.set_mask(T_const.GC_EMPTY_STRING)
        if (PC_STATIC_TRACE_NAME_CLASS_NAME.match_trace(i_predefined_trace)) {
            ...
            l_result_trace.set_val(T_const.GC_THREADID)
        } else if (PC_STATIC_TRACE_NAME_PROCESSID.match_trace(i_predefined_trace)) {
            l_result_trace.set_val(T_const.GC_PROCESSID)
    ...
    static ArrayList<I_trace> process_source_exclusive(I_trace i_predefined_source_trace, I_event i_event_config, List<I_trace> i_traces_from_source) {
        ...
        for (I_trace l_trace_from_source : i_traces_from_source) {
            if ((!i_event_config.is_trace_muted(i_predefined_source_trace)) || i_event_config.get_corresponding_trace(l_trace_from_source) != T_const.GC_NULL_OBJ_REF) {
            ...
            for (I_trace l_trace_config : l_event_config.get_traces_config()) {
                if (!l_trace_config.is_muted()) {
                    if (l_trace_config.match_trace(PC_TRACE_SOURCE_PREDEFINED)) {
                        ...
                        if (i_event_runtime.get_throwable() != T_const.GC_NULL_OBJ_REF && i_event_runtime.get_throwable() instanceof E_application_exception) {
                            l_trace_list.addAll(process_source_exclusive(PC_TRACE_SOURCE_EXCEPTION_TRACES, l_event_config, T_s.l().objects2traces(((E_application_exception) i_event_runtime.get_throwable()).get_traces(), T_const.GC_TRACE_SOURCE_EXCEPTION_TRACES)))
                        }
                    } else {
