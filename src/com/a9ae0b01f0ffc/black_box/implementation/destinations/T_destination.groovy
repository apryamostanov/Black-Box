...
//todo: exception stacktrace
    ...
    String p_purpose = GC_EMPTY_STRING
    ...
    static I_trace init_predefined_trace(String i_predefined_trace_name) {
        ...
        l_trace.set_source(GC_TRACE_SOURCE_PREDEFINED)
    ...
    static I_trace find_and_build_trace_inclusive(I_event i_event_runtime, I_trace i_trace_config) {
        ...
        if (l_trace_config_source == GC_EMPTY_STRING || l_trace_config_source == GC_TRACE_SOURCE_ALL || l_trace_config_source == GC_TRACE_SOURCE_RUNTIME) {
        ...
        if (l_trace_config_source == GC_EMPTY_STRING || l_trace_config_source == GC_TRACE_SOURCE_ALL || l_trace_config_source == GC_TRACE_SOURCE_CONTEXT) {
        ...
        if (l_trace_config_source == GC_EMPTY_STRING || l_trace_config_source == GC_TRACE_SOURCE_ALL || l_trace_config_source == GC_TRACE_SOURCE_EXCEPTION_TRACES) {
            if (i_event_runtime.get_throwable() != GC_NULL_OBJ_REF) {
                if (i_event_runtime.get_throwable() instanceof E_application_exception) {
                    for (I_trace l_trace_from_exception : T_s.l().objects2traces(((E_application_exception) i_event_runtime.get_throwable()).get_traces(), GC_TRACE_SOURCE_EXCEPTION_TRACES)) {
    ...
    static I_trace build_predefined_trace(I_trace i_predefined_trace, I_event i_event_runtime, I_trace i_trace_config) {
        ...
        l_result_trace.set_val(GC_EMPTY_STRING)
        l_result_trace.set_source(GC_TRACE_SOURCE_PREDEFINED)
        l_result_trace.set_name(i_predefined_trace.get_name())
        l_result_trace.set_mask(GC_EMPTY_STRING)
    ...
    static void process_source_exclusive(ArrayList<I_trace> i_traces_to_add, I_trace i_predefined_source_trace, I_event i_event_config, List<I_trace> i_traces_from_source) {
        for (I_trace l_trace_from_source : i_traces_from_source) {
            if ((!i_event_config.is_trace_muted(i_predefined_source_trace)) || i_event_config.get_corresponding_trace(l_trace_from_source) != GC_NULL_OBJ_REF) {
            ...
            for (I_trace l_trace_config : l_event_config.get_traces_config()) {
                if (!l_trace_config.is_muted()) {
                    if (l_trace_config.match_trace(PC_TRACE_SOURCE_PREDEFINED.get_ref_class_name(), PC_TRACE_SOURCE_PREDEFINED.get_name())) {
                        ...
                        if (i_event_runtime.get_throwable() != GC_NULL_OBJ_REF && i_event_runtime.get_throwable() instanceof E_application_exception) {
                            process_source_exclusive(l_trace_list, PC_TRACE_SOURCE_EXCEPTION_TRACES, l_event_config, T_s.l().objects2traces(((E_application_exception) i_event_runtime.get_throwable()).get_traces(), GC_TRACE_SOURCE_EXCEPTION_TRACES))
                        }
                    } else {
