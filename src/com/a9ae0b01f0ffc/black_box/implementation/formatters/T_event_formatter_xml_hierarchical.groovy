...
class T_event_formatter_xml_hierarchical extends T_event_formatter {
    ...
    String format_event(T_event i_source_event) {
        ...
        if ([GC_EVENT_TYPE_METHOD_ENTER, GC_EVENT_TYPE_STATEMENT_ENTER, GC_EVENT_TYPE_EXPRESSION_ENTER].contains(l_event_type)) {
            l_result += open_tag(i_source_event.get_execution_node().get_name_xml(), attrs(attr(PC_ATTR_DATETIMESTAMP, i_source_event.get_datetimestamp()), attr(PC_ATTR_NAME, i_source_event.get_execution_node().get_name()), attr(PC_ATTR_LINE, i_source_event.get_line_number().toString())))
            ...
            if ((![GC_EVENT_TYPE_INFO, GC_EVENT_TYPE_WARNING].contains(i_source_event.get_event_type())) && method_arguments_present(i_source_event.get_traces_standalone())) {
                l_result += open_tag(i_source_event.get_event_type(), attrs(attr(PC_ATTR_DATETIMESTAMP, i_source_event.get_datetimestamp()), attr(PC_ATTR_MESSAGE, i_source_event.get_message()), attr(PC_ATTR_LINE, i_source_event.get_line_number().toString())))
                ...
                l_result += open_tag(i_source_event.get_event_type(), attrs(attr(PC_ATTR_DATETIMESTAMP, i_source_event.get_datetimestamp()), attr(PC_ATTR_LINE, i_source_event.get_line_number().toString())))
                l_result += make_traces(i_source_event.get_traces_standalone(), PC_TRACE_TAG_NAME_TRACE)
                l_result += close_tag(i_source_event.get_event_type())
            } else {
                l_result += tag(i_source_event.get_event_type(), attrs(attr(PC_ATTR_DATETIMESTAMP, i_source_event.get_datetimestamp()), attr(PC_ATTR_MESSAGE, i_source_event.get_message()), attr(PC_ATTR_LINE, i_source_event.get_line_number().toString())))
            }
        }
        return l_result
    }
